@tool
extends Node3D
## Sprint 28sexdecies : grille de demonstration - genere plusieurs
## DwarfModel3D (scripts/prototypes/DwarfModel3D.gd) avec des variations
## aleatoires INDEPENDANTES chacune, cote a cote dans une grille, pour
## comparer d'un coup plusieurs nains sans avoir a cliquer "Randomiser" un
## par un dans l'Inspecteur.
##
## Sprint 28sexagesies : outil de verification UNIFIE, demande par
## l'utilisateur ("on va unifier les prototypes pour vérifier tout ça") -
## remplace le besoin d'alterner entre le prototype a un seul nain
## (DwarfModel3DPrototype.tscn, toujours utile pour des reglages fins a la
## main) et cette grille. Mode par defaut "Verification complete" : grille
## 6x6 (36 nains), chacun entierement randomise (apparence + armes +
...
## Sprint 28sexagesies : les 6 valeurs possibles de preview_animation (voir
## DwarfModel3D.gd) - utilisees en mode "Verification complete" pour assigner
## une animation differente a chaque COLONNE de la grille (voir _regenerate),
## afin de voir toutes les actions jouer en meme temps sur des nains varies.
const ANIM_STATES := ["Aucune", "Marche", "Travail", "Combat", "Manger", "Dormir"]

## Sprint 28quinvicies : 9 configurations d'armes fixes (pas aleatoires) -
## couvrent les 5 categories demandees (1 main / 2 mains / 1 main + bouclier /
## deux armes 1 main / distance), en Repos ET en Combat quand la categorie le
## permet (8 cases), plus l'arme a distance en Repos pour la 9e case. Utilisee
## par _apply_weapon_demo() quand grid_mode = "Demonstration armes".
...
## WEAPON_DEMO_CONFIGS ci-dessous. "Verification complete" (Sprint
## 28sexagesies, defaut) : unifie les deux prototypes en un seul outil de
## verification - CHAQUE case est entierement randomisee (apparence + armes +
## materiau) ET chaque COLONNE joue une animation differente parmi les 6 de
## ANIM_STATES (Aucune/Marche/Travail/Combat/Manger/Dormir), pour voir a la
## fois la variete des tirages et toutes les actions d'animation en meme
## temps. Pensee pour une grille 6x6 (36 = 6 variations x 6 animations par
## colonne), voir grid_columns/grid_rows par defaut plus bas.
...
@export var spacing: float = 2.0  # elargi (etait 1.8) - les armes 2 mains/boucliers sont plus grosses depuis le retour utilisateur sur leur taille
		...
		for col in range(grid_columns):
			...
			match grid_mode:
				...
				"Verification complete":
					# Sprint 28sexagesies : tirage complet (apparence + armes,
					# voir DwarfModel3D._randomize_variation) PUIS l'animation
					# est forcee selon la colonne (pas laissee au hasard), pour
...
## Sprint 28quinvicies : applique la configuration d'armes n°"index" (voir
## WEAPON_DEMO_CONFIGS plus haut) sur l'instance "dwarf" donnee. Les champs
## d'armes de DwarfModel3D (weapon_loadout/weapon_type/shield_type/
## ranged_type/weapon_pose) sont des variables @export publiques (le prefixe
## "_" de ce script n'existe que sur ses propres methodes), donc modifiables
## directement depuis ici avant d'appeler _rebuild(). Retourne le texte a
