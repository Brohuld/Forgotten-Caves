extends RefCounted
## 2026-07-06 (revue de code, paquet E, M58 - etape 1/3) : extrait de
## DwarfModel3D.gd (23 fonctions liees a la construction/pose des armes -
## _build_weapons et tout ce qu'elle appelle, directement ou indirectement).
## Fichier source ~2060 lignes, largement au-dessus du seuil de l'axe 1 -
## decoupe en 3 etapes testees separement (armes ici, puis tenue/armure, puis
## cheveux/barbe) plutot qu'un seul gros changement d'un coup. Aucun
...
##   "Repos" (ceinture/dos) via add_child - le script appelant passera "self".
##
## Voir Model3DUtils.gd pour flat_material()/edited_owner() (utilitaires
## partages, copies de DwarfModel3D._flat_material/_edited_owner).
...
## copie de DwarfModel3D.MATERIAL_COLORS (un const n'est pas accessible via
## get() sur "model", donc duplique ici comme les champs de Model3DUtils).
...
## Sprint 28quinvicies : choisit et place l'equipement selon
## "weapon_loadout" (voir @export_enum "Armes" dans DwarfModel3D.gd), puis les
## place selon effective_weapon_pose() : "Repos" -> attach_to_belt (armes a
## une main) ou attach_to_back (armes a 2 mains/boucliers/armes a distance) ;
## "Combat" -> attach_to_hand (main droite = arme principale, main gauche =
## bouclier/2e arme).
...
## 2026-07-06 (revue de code, paquet E, I63) : les 5 fonctions ci-dessous sont
## extraites des branches du match de build_weapons(), aucun changement de
## comportement - meme pattern que le dispatch par type deja utilise pour
## Dwarf.gd/_complete_task (revue de code Phase 4, C17).
...
## build_weapons pour recalculer weapon_color a chaque reconstruction (le
## champ n'est plus directement exportable/editable, voir sa declaration dans
## DwarfModel3D.gd).
...
## rotation lors de l'attache (ceinture/dos/main, voir attach_to_*).
...
## attach_to_hand) est maintenant au MILIEU de la poignee au lieu de tout en
...
## 2026-07-06 (revue de code Phase 3, C18) : decoupee en une fonction par
## type d'arme (make_mace_head/make_axe_blade/make_sword_blade) + un helper
## commun (weapon_handle_length_base/make_weapon_handle) - depassait tres
## largement le seuil de 50 lignes de l'axe 1. Meme pattern que les styles de
## cheveux de ce meme fichier. Aucun changement de comportement.
static func make_weapon_model(model: Node3D, kind: String, two_handed: bool) -> Node3D:
	...
	# taille nettement plus lisible, coherente avec une arme tenue a 2 mains.
	...
	# groupe (0,0,0) est le point attache a la main (voir attach_to_hand) :
...
## Sprint 28octovicies : longueur de manche desormais PAR TYPE (avant, une
## seule valeur pour toutes les armes) - la masse avait l'air "cassee"
...
## 2026-07-06 (revue de code Phase 3, C18) : extrait de make_weapon_model().
static func weapon_handle_length_base(kind: String) -> float:
	match kind:
		"Masse":
			return 0.42
		"Hache":
			return 0.34  # etait 0.30, legere augmentation (voir aussi la lame ci-dessous)
		_:  # "Epee"
			return 0.22


## 2026-07-06 (revue de code Phase 3, C18) : extrait de make_weapon_model().
## 2026-07-06 (bug) : n'assigne plus "owner" ici (ni dans les autres make_*/
## build_* ci-dessous) - "group" n'est pas encore accroche a son parent final
## a ce stade, donc Godot refusait l'assignation ("Invalid owner. Owner must
## be an ancestor in the tree."). L'owner de TOUT le sous-arbre est desormais
## assigne d'un coup par Model3DUtils.adopt_recursive(), une fois le groupe
## reellement attache (voir attach_to_belt/attach_to_back/attach_to_hand).
## Aucun changement visuel - "owner" ne sert qu'a la persistance/visibilite
## dans le panneau Scene de l'editeur.
...
static func make_mace_head(model: Node3D, group: Node3D, handle_top: float, scale_factor: float) -> void:
	...
	# Petites "flanges" (fines boites autour de la tete) pour lire clairement
	# comme une masse d'armes plutot qu'une simple boule.
	for i in range(4):
...
static func make_axe_blade(model: Node3D, group: Node3D, handle_top: float, scale_factor: float) -> void:
	...
	# Sprint 28quinsexagesies : la hache 1 main etait trop petite, signale par
	# l'utilisateur - lame nettement agrandie (etait 0.14, 0.17, 0.02) et
	# decalee un peu plus loin du manche pour rester lisible a la nouvelle
	# taille.
...
static func make_sword_blade(model: Node3D, group: Node3D, handle_top: float, scale_factor: float) -> void:
	# Sprint 28sepsexagesies : epee 1 main jugee trop fine/courte - lame
	# allongee (0.52 -> 0.64) et epaissie (largeur 0.035 -> 0.05, epaisseur
	# 0.01 -> 0.018).
...
## bouche/les sourcils (_build_curve_segments dans DwarfModel3D.gd), mais
## construit ici directement dans le groupe local (pas sur le modele) pour
## que la courbe reste attachee/bouge avec l'arme lors du positionnement.
...
## 2026-07-06 (revue de code, paquet E, I63) : extrait de make_ranged_model()
## - branche "Arbalete", aucun changement de comportement.
...
## 2026-07-06 (revue de code, paquet E, I63) : extrait de make_ranged_model()
## - branche "Arc", aucun changement de comportement.
...
static func attach_to_back(model: Node3D, parent: Node, item: Node3D, _head_y: float, is_ranged: bool = false) -> void:
	...
	# 2026-07-06 (revue de code, paquet H, I66/I67) : TENTATIVE d'ajustement -
	# constat de Francois pendant M58 : armes de melee "trop inclinees et trop
	# basses" en position Repos dans le dos, arc/arbalete "mal accroches".
	# Hauteur relevee (0.85 -> 0.92 * torso_height, plus proche de l'epaule) et
	# inclinaison reduite (-155 -> -168 deg, plus proche de la verticale, donc
	# moins "incline") pour les DEUX cas. Ceci reste une estimation a l'oeil
	# (geometrie 3D, point faible connu - voir memoire projet) : a verifier et
	# corriger en jeu par Francois plutot qu'a considerer comme regle definitive.
	var height_factor: float = 0.92
	var tilt_deg: float = -168.0
	if is_ranged:
		# Distinction ajoutee ici (nouveau parametre) pour permettre un reglage
		# independant arc/arbalete vs armes de melee/bouclier a l'avenir - pour
		# l'instant, meme formule que le melee (aucune donnee visuelle propre
		# a l'arc/l'arbalete pour deviner un meilleur reglage sans capture
		# d'ecran).
		height_factor = 0.92
		tilt_deg = -168.0
...
## bout du pivot de bras, voir _build_arms dans DwarfModel3D.gd), avec une
## legere orientation pour paraitre "empoignee" plutot que de pendre droit
## vers le bas. Un bouclier garde sa rotation neutre (deja face a +Z par
## construction, voir make_shield_model). Ne prend pas "model" en parametre :
## ne lit/n'ecrit aucun champ d'apparence, seulement la main deja fournie.
static func attach_to_hand(item: Node3D, hand: Node3D, is_shield: bool) -> void:
	if not hand:
		return
	if not is_shield:
		# Sprint 28septvicies : le signe etait inverse - le personnage fait
		# face a +Z (voir eye_z/nose/bouche dans _build_face de
		# DwarfModel3D.gd, tous positifs), or rotation.x = -70 deg envoie la
		# lame vers -Z (l'arriere), signale par l'utilisateur ("les armes
		# pointent en arriere"). +70 deg envoie la lame vers +Z (l'avant, cote
		# visage) avec une legere inclinaison vers le haut - corrige.
...
## droite tient deja le grip (voir attach_to_hand) ; on fait aussi pivoter le
## bras gauche pour l'amener pres du manche (desormais bien plus long, voir
## make_weapon_model), comme si les deux mains le portaient ensemble.
...
## donc le bouclier (attache a Hand_L, voir attach_to_hand) se retrouvait
## fondu/enfonce dans le torse au lieu de ressortir devant. On leve le bras
## gauche vers l'avant (meme principe que pose_two_handed_grip), pour que le
## bouclier se degage nettement du corps. Sans effet si preview_animation !=
## "Aucune" (voir _process dans DwarfModel3D.gd, qui reprend la main sur les
## pivots de bras).
static func pose_shield_arm(arm_pivot_l: Node3D) -> void:
	if not arm_pivot_l:
		return
	arm_pivot_l.rotation.x = deg_to_rad(-72.0)  # etait -55, encore fondu dans le corps signale par l'utilisateur
