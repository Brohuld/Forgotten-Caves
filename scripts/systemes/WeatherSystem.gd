extends Node
## Systeme meteo purement visuel, independant du gameplay (comme
## DayNightCycle.gd). Alterne aleatoirement dans le temps entre Normal /
## Brouillard / Pluie / Neige. Ajoute les effets meteo PAR-DESSUS ce que
## DayNightCycle calcule (densite de fog, energie lumineuse, teinte du ciel) :
## DayNightCycle reste proprietaire des couleurs de ciel/sol et de la
## rotation/couleur de base de la lumiere ; WeatherSystem n'ajoute qu'un
## delta de densite de fog, un multiplicateur d'energie lumineuse et une
## teinte de ciel, tous lisses/interpoles pour eviter un changement brutal
## quand la meteo change.
##
## Cette composition ne fonctionne QUE parce que WeatherSystem est place
## APRES DayNightCycle dans Main.tscn (donc son _process() tourne juste
## apres) - un reordonnancement accidentel des noeuds casserait
## silencieusement le resultat visuel (aucune erreur levee) si WeatherSystem
## relisait _light.light_energy/_sky_material.*_color directement.
## WeatherSystem lit donc les champs publics base_light_energy/
## base_sky_*_color/base_ground_*_color de DayNightCycle (voir
## _day_night_cycle ci-dessous) plutot que ces valeurs potentiellement deja
## modifiees par lui-meme - la composition ne depend ainsi plus de l'ordre
## des noeuds.
...
## ici - une constante de hauteur dupliquee et jamais mise a jour ailleurs
## causerait un bug silencieux (deja vecu par le passe).
const VoxelWorldScript := preload("res://scripts/monde/VoxelWorld.gd")
## Voir SeasonSystem.gd - meme pattern pour lire DayNightCycleScript.game_speed
## (pause/x1/x2/x4).
const DayNightCycleScript := preload("res://scripts/systemes/DayNightCycle.gd")
## Pour season_id_or_default().
...
## Force du melange vers SKY_TINT_COLOR - suffisamment elevee pour donner un
## changement de couleur de ciel clairement visible selon la meteo.
...
## Reference au noeud (pas juste au script comme DayNightCycleScript
## ci-dessus) pour lire ses champs d'instance base_light_energy/
## base_sky_*_color/base_ground_*_color - voir le commentaire d'en-tete de
## ce fichier.
@onready var _day_night_cycle: Node = %DayNightCycle
var _rain_particles: GPUParticles3D
var _snow_particles: GPUParticles3D

# La meteo n'est pas tiree dans une seule liste fixe - chaque saison a sa
# propre "urne" de probabilites (repetition = plus de chances), pour que
# l'hiver ait plus de neige, le printemps/l'automne plus de pluie, et l'ete
# quasiment jamais de neige.
...
func _process(delta: float) -> void:
	...
	# Teinte le ciel (deja colore par DayNightCycle ce meme frame, voir
	# commentaire de SKY_TINT_COLOR) vers un gris selon la meteo courante.
...
func _pick_new_weather() -> void:
	# L'urne de probabilites depend de la saison courante (SeasonSystem.gd) -
	# repli sur la saison par defaut si jamais SeasonSystem n'est pas trouve
	# (ne devrait pas arriver, garde-fou uniquement), factorise via
...
## (TemperatureSystem.gd, ClimateUI.gd) n'aient pas besoin de resoudre l'enum
## "Weather" via une reference generique %WeatherSystem (le typage generique
## ne resout pas les enums definis dans ce script, meme probleme deja
## rencontre avec VoxelWorld.BlockType - voir ses commentaires).
...
## Accesseurs utilises par CloudSystem.gd pour teinter les nuages EN PHASE
## avec la teinte du ciel (meme couleur/force que SKY_TINT_COLOR/
...
## Couleur utilisee pour la petite icone meteo (voir IconRenderer.make_weather_icon)
